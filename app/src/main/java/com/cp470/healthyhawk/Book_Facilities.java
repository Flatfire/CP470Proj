...
public class Book_Facilities extends AppCompatActivity {
    ...
    /**
     * Layout generation and event handling for Facility Booking activity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pickEndHour.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            /**
             * Perform time checks on selected hour
             * @param adapterView
             * @param view
             * @param i
             * @param l
        ...
        db.child(BUILDINGS).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            /**
             *
             * Get data for Building spinner and form basis for secondary spinner data
             *
             * @param task
             */
            @Override
        ...
        buildingSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            /**
             * Trigger for gathering facility spinner data based on currently selected building
             * @param adapterView
             * @param view
             * @param i
             * @param l
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                facilityArr.clear();
                db.child(buildingSpinner.getSelectedItem().toString()).child("facility").get().addOnCompleteListener(task -> {
                    if (!task.isSuccessful()) {
                        Log.e("firebase", "Error getting data", task.getException());
                    } else {
        ...
        /**
         * Determines calendar date selection for CalendarView element
         */
        ...
        /**
         * Initiates booking routines to capture user input and perform
         * booking sanity checks.
         */
        bookingSubmit.setOnClickListener(view -> {
    ...
    /**
     *
     * Designed to convert 12 hour time captured from user input
     * into 24-hour time that is more useful for direct comparison
     * and storage.
     *
     * @param hour selected hour
     * @param minute selected minute
     * @param meridiem selected AM/PM
     * @return Formatted time string for storage and manipulation
     */
    ...
    public class book extends AsyncTask<Void, Void, Void> {
        /**
         * Perform background booking tasks
         * @param voids
         * @return
         */
        @Override
        protected Void doInBackground(Void... voids) {
            ...
            db.child(bookBuilding).child("facility").child(bookFacility).child("bookings").child(bookDay).get().addOnCompleteListener(task -> {
                ...
                if (!task.isSuccessful()) {
                    ...
                    if (!bookingConflict) {
                        ...
                        // Return to home screen if pressed
                        greatSuccess.setPositiveButton("Exit", (dialogInterface, i) ->  {
                            finish();
                        });

                        // Allow user to continue booking if this button is pressed
                        greatSuccess.setNegativeButton("Book Another", (dialogInterface, i) ->  {});
